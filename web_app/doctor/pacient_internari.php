...
<head>
	...
	<span ><?php 
	   ...
	   echo '<a class="btn btn-info bg-primary" style="margin-left:1410px;" href="adaugare_internare.php?' . $items_add . '" class="btn btn-success">Adaugare internare</a>';?></span>
	</div>
</head>
<body>
<body style="background-image:url('sfat.jpg');background-repeat:no-repeat;background-size:cover;">
	...
	<thead>
		<tr>
		  ...
		  <th scope="col" style="width:300px;">Actiuni</th>
		</tr>
	</thead>
	<tbody>
		<?php
				...
				if(isset($_POST['update_internari'])){
					...
					$query_update = "UPDATE internari SET data_internare='$data_internare', data_externare='$data_externare',diagnostic_internare='$diagnostic_internare',diagnostic_externare='$diagnostic_externare', tratament='$tratament', spital='$spital' WHERE id= $id_internare;";
					$results_update = mysqli_query($db, $query_update);
					$query = "SELECT id,data_internare, data_externare,diagnostic_internare, diagnostic_externare,tratament, spital FROM internari WHERE id_pacient = $id_pacient ";
					$results = mysqli_query($db, $query);
					$i=0;
					if(mysqli_num_rows($results) > 0 ){
						while($row = mysqli_fetch_assoc($results)){?>
							<tr>
								...
								echo '<a style="width:65px" href="edit_action_internari.php?' . $items_edit . '"
								...
								$items = implode('&', $files);
								echo '<a href="delete_action_internari.php?' . $items . '" class="btn btn-danger">Ștergere</a>';
								?>
								</td>
							</tr>
					<?php
