...
<form action="grafic_greutate.php" method = "post">
	<p >Selecteaza intervalul de timp:</p>
	<p> De la :<input type="date" name="data_inceput">pana la<input type="date" name="data_sfarsit"></p>
	<input type="submit" class="btn btn-success btn-xs" name="interval_timp" value="Confirmare">
</form>
</div>
...
$results = mysqli_query($db, $query);
	...
	while($row = mysqli_fetch_assoc($results)){
		...
		//if(isset($_POST["interval_timp"])){
			...
			$sistolica=(int)($t[0]);
			$diastolica=(int)($t[1]);
			$variable = "<script>document.write(data1)</script>";
			//$sistolica1=(string)($sistolica);
			$p=$_POST['section'];
			//echo $variabile1;
			$d=date('d-m-Y', strtotime($row['data']));
			$dataPoints[]=array("x" =>date($d), "y" =>$sistolica);	
