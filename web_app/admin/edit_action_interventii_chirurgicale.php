<head>
	...
	<nav class="navbar navbar-expand-md navbar-dark bg-primary" style="height:60px;">
			...
			<?php if(isset($_GET['files'])){ echo '<a href="interventii_chirurgicale.php?' . $items_add . '"class="btn btn-info" style="background-color:white;color:blue;" >Înapoi</a>';}?>
...
<body style="background-image:url('pacient2.jpg');background-repeat:no-repeat;background-size:cover;">
	<div class = "card" style="margin:150px 300px 300px 300px; width:1000px;opacity: 0.9;">
		...
		<div class="card-body" style="font-size:26; background-color:#ccccb3;">
			<form action="interventii_chirurgicale.php" method = "post">

				<?php
				$db = mysqli_connect('localhost', 'root', '', 'patients_manager');
				if(isset($_GET['files'])){
					$id =  $_GET['files'][0];
					$query = "SELECT * FROM interventii_chirurgicale WHERE id=$id";
					$results = mysqli_query($db, $query);
				...
				<div class="right">
					<input type="text" name="id_interventie" value=<?php echo $id;?> readonly="readonly">	
					<input type="text" name="denumire_interventie" value=<?php echo $name;?>>
					<input type="text" name="data_interventie" value=<?php echo $date;?>>
					<input type="text" name="recomandari_interventie" value=<?php echo $recomandari;?>>
					<input type="submit" class="btn btn-success btn-lg" name="update_interventie" value="Salvare">
				</div>
				<?php
				}
				}	
