...
<body style="background-image:url('pacient2.jpg');background-repeat:no-repeat;background-size:cover;">
	<div class = "card" style="margin:150px 300px 300px 300px; width:1000px;opacity: 0.9;">
		<div class = "card-header" style="background-color:#0275d8;">
			<center><h2>Editare boli</h2><center>
	    </div>
		<div class="card-body" style="font-size:26; background-color: #ccccb3">
			<form action="pacient_boli.php" method = "post">

				<?php
				$db = mysqli_connect('localhost', 'root', '', 'patients_manager');
				if(isset($_GET['files'])){
					$id =  $_GET['files'][0];
					$query = "SELECT PB.id, B.denumire_boala, PB.data, PB.tratament FROM boli B, pacienti_boli PB WHERE PB.id=$id AND B.id = PB.id_boala";
					$results = mysqli_query($db, $query);
					if(mysqli_num_rows($results) > 0 ){
						while($row = mysqli_fetch_assoc($results)){
							$id=$row['id'];
							$name=$row['denumire_boala'];
							$date=$row['data'];
							$medicine=$row['tratament'];
						}
				?>
				<div class="left">
					<span>Denumire boala:</span>
					<span >Data:</span>
					<span >Tratament:</span>
				</div>
				<div class="right">
					<input type="hidden" name="id_boala" value=<?php echo $id;?> readonly="readonly">	
					...
					<input type="date" name="data_boala" value=<?php echo $date;?>>
					<input type="text" name="tratament" value=<?php echo $medicine;?>>
					<input type="submit" class="btn btn-success btn-lg" name="update_boala" value="Salvare">
				</div>
				<?php
				}
				}	
