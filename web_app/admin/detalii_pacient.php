...
<head>
	...
	<nav class="navbar navbar-expand-md navbar-dark bg-primary" style="height:60px;">
		...
		<?php
			...
			$query = "SELECT nume, prenume FROM medici where id = $id_medic";
			$results = mysqli_query($db, $query);
		...
		<a href="acasa_admin.php?admin=<?php echo $_GET['files'][1];?>" class="btn btn-info" style="background-color:white;color:blue;"><i class="fa fa-fw fa-home"></i>Acasa</a>
...
<body style="background-image:url('pacient2.jpg');background-repeat:no-repeat;background-size:cover;">
	<ul style="padding-top:15px;padding-bottom:15px;">
		...
		<li><a href="" class="btn btn-info bg-primary" style="margin-right:10px;">Informatii generale</a></li>
		<li><?php echo '<a class="btn btn-info bg-primary" style="margin-right:10px;" href="pacient_boli.php?' . $items . '">Istoric boli</a>' ;?></li>
		<li><?php echo '<a class="btn btn-info bg-primary" style="margin-right:10px;" href="pacient_internari.php?' . $items . '">Istoric internari</a>'; ?></li>
		...
		<li><?php echo '<a class="btn btn-info bg-primary" style="margin-right:10px;" href="interventii_chirurgicale.php?' . $items . '">Interventii chirurgicale</a>'; ?></li>
		...
		<li><?php
			...
			$items = implode('&', $files);
			echo '<a class="btn btn-info bg-primary" style="margin-right:10px;" href="pacient_consultatii.php?' . $items . '" >Consultatii</a>';
	...
	<div class="container-fluid" style="padding:50px 400px 50px 300px;opacity: 0.9;" >
		<div class="card" style="background-color:#ff794d;padding:20px 0px 20px 40px;border-radius:20px;">
			<div class="card-body" style=" color:black;font: italic bold  16px Georgia, serif;">
				<h5 class="card-title" style ="font: italic bold  30px Georgia, serif;color:black">Informatii generale</h5>
				<?php
				$username = "";
				$email    = "";
				$errors = array(); 
				...
				$db = mysqli_connect('localhost', 'root', '', 'patients_manager');
				//$identif = $_GET['subject'];
				$query = "SELECT id, nume, prenume, cnp, data_nastere, adresa, telefon, email, asigurat,denumire_cas, grupa_sange,rh  FROM pacienti where id = $identif";
				$results = mysqli_query($db, $query);
