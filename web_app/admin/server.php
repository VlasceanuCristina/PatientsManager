<?php 
	...
	if (isset($_POST['login_user'])) {
		...
		if (count($errors) == 0) {
			...
			$query = "SELECT * FROM administratori WHERE email='$username' AND password='$password'";
			$results = mysqli_query($db, $query);
			$row= mysqli_fetch_array($results);
			$id_admin = $row['id'];
			$_SESSION['nume_admin']=$row['prenume'].'(admin)';
			$_SESSION['id_admin']=$id_admin;
			if($id_admin){
			header('location: acasa_admin.php?admin='.$id_admin);
			}
			else {
