...
<div class = "card"  style="margin:150px 300px 300px 300px; width:1000px;opacity:0.9;">
	<div class = "card-header" style="background-color:#0275d8;">
		<center><h2>Adăugare vaccin</h2><center>
	</div>
	<div class = "card-body" style="font-size:26; background-color:#ccccb3;">
		<form method="post" action="adaugare_vaccin_action.php">
			<div class="left">
				<span>Denumire:</span>
				<span>Data:</span>
			</div>
			<div class="right">
				<select name="denumire_vaccin">
							<?php $db = mysqli_connect('localhost', 'root', '', 'patients_manager');
								$query_vaccin="SELECT * FROM vaccinuri";
				...
				<input type= "date" style="height:50px;" name = "data_vaccin"> 
				<?php if(isset($_GET['files'])){?>
				<input type="hidden" name="identificare" value=<?php echo $items_add;?>>
				<?php }?>
				<input type="submit" name = "adaugare_vaccin" class="btn btn-success"  style = " width: 100px;height:40px;text-align: center;"value = "Adauga"/>
			</div>
	</div>
</div>
</body>