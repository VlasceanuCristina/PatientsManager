<?php 
	...
	if (isset($_POST['reg_user'])) {
		...
		if (count($errors) == 0) {
			...
			header('location: login.php');
				...
				array_push($errors, "Combinatia nume de utilizator/parola nu este corecta!");
			}
		}
	...
	if (isset($_POST['login_user'])) {
		...
		if (empty($username)) {
			array_push($errors, "Completeaza numele de utilizator!");
		}
		if (empty($password)) {
		...
		if (count($errors) == 0) {
			...
			$query = "SELECT * FROM pacienti WHERE email='$username' AND password='$password'";
			$results = mysqli_query($db, $query);
			$row= mysqli_fetch_array($results);
			$id_pacient = $row['id'];
			if($id_pacient){
			header('Location:acasa_pacient.php?pacient='.$id_pacient);
			}
			else{
